...
type Step = { order: number; imageUrl?: string; videoUrl?: string; text?: string };
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-4">
          ...
          {steps.map((s, idx) => (
            <motion.div key={idx} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} className="border rounded p-3 mb-2">
              <div className="flex gap-2">
                <input className="flex-1 border rounded p-2" placeholder="Image URL (or use Upload)" value={s.imageUrl || ""} onChange={(e) => setSteps((arr) => arr.map((it, i) => i === idx ? { ...it, imageUrl: e.target.value } : it))} />
                <input className="flex-1 border rounded p-2" placeholder="Video URL (optional)" value={s.videoUrl || ""} onChange={(e) => setSteps((arr) => arr.map((it, i) => i === idx ? { ...it, videoUrl: e.target.value } : it))} />
              </div>
      ...
      <div className="flex justify-end">
        <button onClick={onCreate} className="px-3 py-2 bg-black text-white rounded">Save</button>
      </div>
    </div>
  );
}
