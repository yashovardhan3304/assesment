...
type Tour = { id: string; title: string; createdAt: string; isPublic: boolean; shareId: string };
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <Link className="px-3 py-2 bg-black text-white rounded" href="/editor">New Tour</Link>
      </div>
      <div className="mt-2 grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div className="border rounded p-3">
          ...
          <div className="text-2xl font-bold">{tours.filter(t => t.isPublic).length}</div>
        </div>
        <div className="border rounded p-3">
        ...
        {tours.map((t) => (
          <li key={t.id} className="border p-3 rounded flex items-center justify-between">
            ...
            <div className="space-x-2">
              <Link className="underline" href={`/editor/${t.id}`}>Edit</Link>
              {t.isPublic && <Link className="underline" href={`/p/${t.shareId}`}>Public link</Link>}
            </div>
          </li>
        ))}
      </ul>
